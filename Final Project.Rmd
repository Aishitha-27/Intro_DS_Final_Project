...
```{r}
# Check the names of columns in cleaned_data
...
We build **Linear Regression** to start off with because this question focuses on predicting total sales revenue using several potential explanatory variables like discounts, quantity sold, order priority, payment methods, and customer region.
...
```
We evaluate how well our linear regression model is performing in predicting total sales revenue based on certain variables. 
```{r}
...
```
The model's evaluation metrics suggest moderate performance in predicting total sales revenue. The **R-squared** value of 0.428 indicates that about 43% of the variance in sales revenue is explained by the model, meaning there is room for improvement. The **MAE** of 621.52 suggests that, on average, the model's predictions are off by around 621 units, while the **RMSE** of 826.14 indicates that larger prediction errors are more significant. Overall, the model provides some useful insights, but its accuracy could be improved by adding more predictors or trying different modeling approaches.
...
```
The regression tree revealed that the **most influential variable** was **total quantity sold**.The first split occurred at a threshold of 26.5 units sold, highlighting its critical role in determining sales revenue. Subsequent splits were based on other variables, but their contribution to prediction improvement was relatively minor. This is suggesting that sales revenue is heavily driven by the quantity sold, with other factors playing a secondary role.
...
rmse_value <- rmse(comparison$Actual, comparison$Predicted)
mape_value <- mape(comparison$Actual, comparison$Predicted)

cat("MAE:", mae_value, "\nRMSE:", rmse_value, "\nMAPE:", mape_value)
```
The tree struggles with accurately predicting extreme values or variations in the data.
...
```
The heatmap reveals that **total quantity sold** has a strong positive correlation (0.65) with **total sales revenue**, indicating it is a key driver of revenue. In contrast, **average discount** shows negligible correlation with both revenue and quantity sold, suggesting discounts may not significantly impact sales performance. This highlights the need to focus on other factors or strategies to optimize revenue.
...
```
This scatter plot shows the relationship between total quantity sold and total sales revenue. There is a clear positive linear trend, as higher quantities sold generally correspond to higher sales revenue. The red line represents the regression fit, indicating a strong linear correlation between the two variables.
