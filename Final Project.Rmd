...
We applied the SMART framework to classify customers into Low, Medium, and High value segments based on their purchase frequency, product preferences, and payment methods.

**specific**: Can we classify customers into Low, Medium, and High value segments based on their purchasing behavior?

**Measurable**: How will we measure the success of this classification using a classification tree model?

**Achievable**: Is it feasible to segment customers based on total spend and other purchasing features using existing data?

**Relevant**: How does classifying customers help businesses tailor marketing strategies for high-value customers?

**Time-bound**: Can we complete the customer segmentation and analysis within the project's timeline using a training and testing approach?

We applied the SMART framework to answer the question: How can we classify customers into value segments based on their purchasing behavior? We used a classification tree model to segment customers by total spend, purchase frequency, and preferences, with total_quantity being the most significant predictor for classification.


## Exploratory Data Analysis 

Exploratory Data Analysis (EDA) is like taking a closer look at the puzzle pieces before trying to fit them together. It’s the phase where we get to know the data, spot anything unusual, and start to see the bigger picture.

For this project, we kicked things off by getting familiar with the dataset. We took a good look at the sales data by country, explored how returns were distributed, and identified the top-selling products. We also noticed some interesting trends, like how sales varied over time and which payment methods were most popular. Visualizing this information with charts helped us spot key patterns, such as peak sales during certain months and how returns spiked in specific categories.

EDA also helped us think about the right questions to ask: How do things like customer location or payment methods influence purchasing decisions? Why are returns higher for certain products? This stage of the project set us up for deeper analysis and gave us a clearer sense of direction for the next steps.
...
cat("Most Returned Item:\n")
print(most_returned_item)

cat("\nMost Not-Returned Item:\n")
...
# Calculate the purchase frequency for each item (StockCode)
purchase_frequency <- cleaned_data %>%
  group_by(StockCode) %>%
  summarise(PurchaseFrequency = n())

# View the resulting data
head(purchase_frequency)
```
...
```{r}
# Group the data by product (Description) and customer type (Country)
...
# Step 1: Create a time variable (e.g., year-month) and summarize data
sales_over_time <- cleaned_data %>%
  mutate(TimePeriod = floor_date(as.Date(InvoiceDate), "month")) %>%  # Group by month
  group_by(TimePeriod, Product = Description) %>%
  summarize(TotalSales = sum(Quantity, na.rm = TRUE), .groups = 'drop')

# Step 2: Line plot showing product sales over time
ggplot(sales_over_time, aes(x = TimePeriod, y = TotalSales, color = Product)) +
  geom_line(size = 1) +
  labs(
