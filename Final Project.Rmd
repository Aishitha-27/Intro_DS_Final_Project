...
Assessing normality is critical because many statistical methods and tests assume data follows a normal distribution. Comparing observed distributions to theoretical ones, such as the normal distribution, helps verify these assumptions. If data deviates significantly, as seen in the Q-Q plots for sales revenue and discounts, transformations (e.g., logarithmic) can reduce skewness and stabilize variance, making data suitable for modeling and hypothesis testing.
...
The analysis conducted provided valuable insights into the dynamics of online sales and customer behavior, addressing the SMART questions posed for this project. The findings revealed clear patterns in sales performance, customer segmentation, and operational dynamics.

*Sales Patterns:* Total quantity sold emerged as the most significant driver of sales revenue, while discounts showed minimal impact, suggesting a focus on strategies to boost sales volume. Regional and product-based trends emphasized the importance of understanding customer preferences for targeted marketing efforts.

*Return Dynamics:* Analysis of return patterns revealed no significant differences across product categories. However, improving return policies or product quality could enhance customer satisfaction and reduce return rates.

*Customer Segmentation:* Classification models highlighted clear customer segments, with high spenders strongly associated with larger quantities purchased. The segmentation offered actionable insights for prioritizing marketing strategies toward high-value customers.

*Operational Insights:* Order prioritization was identified as a significant factor influencing sales channels (online vs. in-store), though the logistic regression model’s low accuracy indicates a need for additional features like customer demographics or seasonal trends to better understand these dynamics.

*Feature Importance:* Across all models, total quantity sold consistently emerged as the most impactful factor, with operational variables like order priority showing limited but notable influence on outcomes.

*Note:* While the findings provide meaningful insights, the observational nature of the data limits our ability to infer causality. There may be unmeasured variables influencing the patterns observed, highlighting the need for further investigation.
