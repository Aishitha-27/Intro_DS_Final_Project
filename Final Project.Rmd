...
cat("MAE:", mae_value, "\nRMSE:", rmse_value, "\nMAPE:", mape_value)
```
The model shows a moderate performance with an MAE of 631.92, indicating an average error of around 632 units in predictions. The RMSE of 830.03 suggests that the model has significant prediction errors, as it penalizes large errors more heavily. The MAPE of 2.16% indicates that, on average, the model's predictions are off by about 2.16% from the actual values, which is a reasonable level of accuracy.
...
```
```{r}
importance_values <- as.data.frame(classification_tree$variable.importance)

var_importance <- data.frame(
  Variable = rownames(importance_values),
  Importance = as.numeric(importance_values$`classification_tree$variable.importance`)
)
...
```{r}
# Ensure that the SalesChannel is encoded as a binary variable (1 for Online, 0 for In-store)
logistic_data <- cleaned_data %>%
  mutate(
    SalesChannel = ifelse(SalesChannel == "Online", 1, 0)  # Encoding: Online = 1, In-store = 0
  ) %>%
  select(SalesChannel, CustomerID, Quantity, UnitPrice, Discount, OrderPriority, Country, PaymentMethod)

# Convert categorical variables to factors for compatibility with the logistic regression model
...
```{R}
library(ggplot2)
coefficients <- broom::tidy(logistic_model)
